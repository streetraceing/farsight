...
function table(rows: Cell[][], headers: string[]): string {
  ...
  const widths = headers.map((_, index) =>
    Math.min(36, Math.max(...data.map((row) => row[index]?.length ?? 0))),
  );
...
function addActivitySection(
  ...
  lines.push(
    table(activityRows(items, limit), [
      label,
      'Commits',
      'Added',
      'Deleted',
...
export function renderReport(report: FarsightReport): string {
  ...
  if (report.project.traits.length)
    lines.push(`Traits: ${report.project.traits.join(', ')}`);
  ...
  if (!report.dependencies.available) {
    ...
    lines.push(
      `Direct dependencies: ${number.format(report.dependencies.totalDeclared)}`,
    );
    if (!report.dependencies.checked) {
      lines.push(
        yellow(report.dependencies.warning ?? 'Dependency check was not run'),
      );
    } else if (report.dependencies.items.length === 0) {
      lines.push(green('No outdated direct dependencies found.'));
    } else {
      lines.push(
        `Needs attention: ${yellow(number.format(report.dependencies.outdatedCount))}`,
      );
      lines.push(
        table(
    ...
    if (report.dependencies.checked && report.dependencies.warning)
      lines.push(dim(report.dependencies.warning));
  }
  ...
  if (report.loc.skippedLargeFiles)
    lines.push(dim(`Skipped large files: ${report.loc.skippedLargeFiles}`));
  ...
  if (extensionRows.length)
    lines.push(table(extensionRows, ['Extension', 'Files', 'Non-empty lines']));
  ...
  if (!report.git.available) {
    ...
    lines.push(
      `Changes: ${green(`+${number.format(report.git.additions)}`)} ${red(`-${number.format(report.git.deletions)}`)}`,
    );
    if (report.git.commits)
      lines.push(
        `Top contributor's commit share: ${percent.format(report.git.topContributorShare)}`,
      );
    ...
    if (report.git.contributors.length) {
      ...
      lines.push(
        `Showing ${number.format(report.git.contributors.length)} of ${number.format(report.git.contributorsCount)}`,
      );
